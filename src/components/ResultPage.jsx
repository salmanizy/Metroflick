...
import { searchMovies, searchActors, searchSeries } from '../api/tmdb';
...
const ResultsPage = () => {
  ...
  useEffect(() => {
    if(keyword){
      Promise.all([searchMovies(keyword), searchActors(keyword), searchSeries(keyword)])
        .then(([movieResult, actorResult, seriesResult]) => {
